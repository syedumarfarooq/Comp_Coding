...
public:
    int climbStairs(int n) {
        ...
        return climbStairs(n-1)+climbStairs(n-2); // in this to reach final stair either u have to be on n-1 or n-2 as it can jump only one or two stairs so we add the combination of reaction n-1 and n-2
    }
};
